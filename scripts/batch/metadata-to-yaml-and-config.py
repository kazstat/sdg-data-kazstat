...
if len(sys.argv) < 2:
    sys.exit('Provide the path of your data config file (eg, config_data.yml).')
...
def convert_metadata(metadata_file, config_file):
    ...
    yaml_metadata_file = metadata_file.replace('.md', '.yml')
    with open(yaml_metadata_file, 'w') as stream:
        yaml.dump(indicator_metadata, stream, allow_unicode=True)
